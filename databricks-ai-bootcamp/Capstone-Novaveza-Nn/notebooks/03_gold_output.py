...
# MAGIC %md
# MAGIC `psycopg2-binary` is already present in the Serverless environment —
# MAGIC deliberately skipping `%pip install` + `restartPython()` here, since that
# MAGIC reinstall/restart cycle is what was hanging the kernel. Just importing it
# MAGIC directly below works fine.
